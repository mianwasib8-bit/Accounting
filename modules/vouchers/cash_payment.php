<?php
/**
 * Cash Payment — 2-row amount entry (row2 = cash/bank)
 */
...
$pageTitle = 'Cash Payment Voucher';
$activePage = 'cpv';
...
<div class="section-head voucher-page-head">
  <div>
    ...
    <p>Row 1 expense/party · Row 2 cash/bank · Amount only · FY <?= e($fy['code'] ?? '—') ?></p>
  </div>
</div>

<form id="voucher-form" class="card accent-left voucher-card" data-type="CPV" novalidate>
  <div class="voucher-meta card-pad">
    <div class="voucher-meta-grid">
      <div class="field">
        <label>Type</label>
        <input class="input input-sm" type="text" value="CPV" readonly disabled />
        <input type="hidden" id="voucher_type" value="CPV" />
      </div>
      <div class="field">
  ...
  <div class="card-pad flex-between flex-wrap voucher-actions">
    <p style="margin:0;font-size:11px;color:var(--muted)">Auto: Expense Dr · Cash Cr</p>
    <div class="flex gap-2">
      <a href="<?= e(url('/modules/vouchers/list.php')) ?>" class="btn btn-secondary btn-sm">Cancel</a>
      <button type="submit" class="btn btn-primary btn-sm" id="btn-save" disabled>Save Payment</button>
    </div>
  </div>
</form>
